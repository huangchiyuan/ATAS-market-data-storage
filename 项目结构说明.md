...
├── demo_realtime_receiver.py    # Python 实时数据接收 Demo（测试用）
│
└── csharp/                      # C# 数据发送端
...
- **`demo_realtime_receiver.py`**：
  - 实时数据接收 Demo：演示如何接收和显示实时数据
  - 不存储数据，仅用于测试和调试
  - 实时显示 Tick 和 DOM 数据，统计接收速率
  - 运行方式：`python demo_realtime_receiver.py`
